...
const schema = defineSchema({
  ...
  appletTransformBatches: defineTable({
    workspaceId: v.number(),
    nodeId: v.string(),
    actions: v.array(
      v.object({
  ...
  }).index("by_workspaceId_nodeId", ["workspaceId", "nodeId"]),
  generator_messages: defineTable({
    workspaceId: v.id("workspaces"),
    appletId: v.string(),
    role: v.union(v.literal("user"), v.literal("assistant")),
    content: v.string(),
  }).index("by_appletId", ["appletId"]),
  generator_builds: defineTable({
    workspaceId: v.id("workspaces"),
    appletId: v.string(),
    prompt: v.string(),
    code: v.string(),
    ...
    .index("by_appletId_createdAt", ["appletId", "createdAt"]) // for recent-first queries
    .index("by_appletId", ["appletId"]),
});
